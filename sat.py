# Agni Ramadani, Fisnik Kuci, Selaudin Agolli
# Since Fisnik visited this course last year the solution has been adopted from his solution from the last year 
...
class SATSearch:
    ...
    def build_parallel_model(self):
        ...
        for time_step in range(self.horizon):
            ...
            # operator exclusion clauses
            for op, op2 in itertools.permutations(self.task.operators, r=2):
                if self.check_conflict(op,op2) or self.check_conflict(op2,op):
                    self.add_clause([self.get_op_var(op, time_step, negated=True),
                                     self.get_op_var(op2, time_step, negated=True)])
    ...
    def check_conflict(self, op1, op2):
        cond1 = op1.del_effects.intersection(op2.preconditions)
        cond2 = op1.del_effects.intersection(op2.add_effects)
        return bool(cond1) or bool(cond2)
